...
export const VariantsDetail: React.FC<VariantsDetailProps> = ({ 
  ...
  const variantComicsStats: ComicStats = {
    totalComics: variantComics.length,
  ...
  // Get unique series for variant comics
  const uniqueSeries = Array.from(new Set(variantComics.map(comic => comic.seriesName))).sort();

  // Sort comics
  const sortedComics = [...variantComics].sort((a, b) => {
    switch (sortBy) {
      case 'series': {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-8">
          ...
          <div className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 p-6">
            ...
            {uniqueSeries.length > 0 && (
              <div className="mt-6">
                <h3 className="text-lg font-semibold text-white mb-3">Variant Comics by Series</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {uniqueSeries.slice(0, 8).map(series => {
                    const seriesCount = variantComics.filter(comic => comic.seriesName === series).length;
                    const seriesValue = variantComics
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-400">{seriesCount} variant{seriesCount !== 1 ? 's' : ''}</p>
                  ...
                  >
                    <div className="relative aspect-[2/3] bg-gray-600">
                      ...
                      <div className="absolute top-1 left-1 flex flex-col space-y-1">
                        <span className="px-1 py-0.5 bg-emerald-500 text-white text-xs font-medium rounded">
                          Variant
                        </span>
                        {comic.isSlabbed && (
